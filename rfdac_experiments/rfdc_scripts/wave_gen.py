...
def createSine(frequency, play_time):
    ...
    new_wave = [
        ...
        for x in range(num_samples * play_time)
    ]
    writeWave(new_wave)
...
def createSaw(frequency, play_time):
    saw_wave = [
        signal.waveforms.sawtooth(2 * np.pi * frequency * x / sampling_rate)
        for x in range(num_samples * play_time)
    ]
    writeWave(saw_wave)
...
def createTriangle(frequency, play_time):
    new_wave = [
        signal.waveforms.sawtooth(2 * np.pi * frequency * x / sampling_rate,
                                  0.5) for x in range(num_samples * play_time)
    ]
    writeWave(new_wave)
...
def createSquare(frequency, play_time):
    new_wave = [
        signal.waveforms.square(2 * np.pi * frequency * x / sampling_rate, 0.5)
        for x in range(num_samples * play_time)
    ]
    writeWave(new_wave)
...
def createWave(frequency, wave_type, play_time):
    if wave_type == 'sine':
        ...
        print(
            'Please type in only waveforms from this list! (sine, saw, triangle, square)'
        )
...
def main(wave_type, frequency, play_time, file_name):
    ...
    if len(file_name) < 4:
        print(
            'your specified filename: \'' + file_name +
            '\' is too short. The file name must end in \'.wav\'.\n Do you want me to change your file name to end in \'.wav\'? (y/n)'
        )
        answer = input()
        if answer == 'y':
